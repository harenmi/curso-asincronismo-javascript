...
// Función que realiza una solicitud HTTP GET a una URL dada y llama a un callback con los datos obtenidos
function fetchData(urlAPI, callback) {
    ...
    // Define una función de devolución de llamada que se ejecuta cuando cambia el estado de la solicitud
    xhttp.onreadystatechange = function (event) {
        // Comprueba si la solicitud se ha completado
        if (xhttp.readyState === 4) {
            // Comprueba si la respuesta es exitosa (código de estado 200)
            if (xhttp.status === 200) {
                // Llama al callback con null como primer argumento (sin error) y los datos obtenidos (parseados como JSON)
                callback(null, JSON.parse(xhttp.responseText));
            } else {
                // En caso de error, crea una instancia de Error y llama al callback con el error y null como argumentos
                const error = new Error('Error' + urlAPI);
                return callback(error, null);
            }
        } 
...
// Realiza una serie de solicitudes encadenadas utilizando la función fetchData
fetchData(`${API}/products`, function (error1, data1) {
    if (error1) return console.error(error1);
    fetchData(`${API}/products/${data1[0].id}`, function (error2, data2) {
        if (error2) return console.error(error2);
        fetchData(`${API}/categories/${data2?.category?.id}`, function (error3, data3) {
            if (error3) return console.error(error3);
            // Imprime en la consola algunos datos obtenidos de las solicitudes
            console.log(data1[0]);          // Imprime los datos del primer producto
            console.log(data2.title);        // Imprime el título del segundo producto
            console.log(data3.name);          // Imprime el nombre de la categoría del segundo producto
        });
    });
});
*/
